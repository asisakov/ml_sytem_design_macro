...
> ## Термины и пояснения
> Ниже приведена таблица с базовой терминологией. Сортировка - по английскому названию.
> 
> | Термин<br/>(англ.)    | Термин<br/>(рус.) | Комментарий                                                                                                                                                                                                                                                            |
> |-----------------------|-----------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
> | Business Requirements | БТ              | Бизнес-требования                                                                                                                                                                                                                                                      |
> | Commodities           |                 | Биржевые товары (энергетика, агро, текстиль, сырьё, металлы, ...)                                                                                                                                                                                                      |
> | Data Scientist        |                 | Роль, отвечает за заполнение соответствующих разделов.<br/>TBD: Совмещает в себе компетенции классического `Data Scientist` с упором на исследования и `ML Engineer` & `ML Ops` роли с акцентом на продуктивизацию моделей                                             |
> | EDA                   |                 | Exploratory Data Analysis - исследовательский анализ данных                                                                                                                                                                                                            |
> | Iteration             | Итерация        | TBD: это все работы, которые совершаются до старта очередного пилота                                                                                                                                                                                                   |
> | Product Owner         |                 | Роль, отвечает за заполнение соответствующих разделов<br/>Примерный круг ответственности: цели и приоритеты, коммуникация со стейкхолдерами, пользовательские истории и требования, аудит беклога, оценка моделей и результатов, управление рисками, KPI и отчётность. |
> |                       |                 |                                                                                                                                                                                                                                                                        |
...
#### 2.1.2 Детализация требований (User Stories)

По детализации:
1. Загружаем признаки - заливаем в базу данных:
1.1. Выбрать источник данных
1.2. Выбрать тип DB
1.3. Выбрать, где будем хостить БД
1.4. Подумать про частоту обновления данных
1.5. В каком виде храним данные? (связано с 1.2)
1.6. Как регулируем доступ к таблице?

2.Обучаем и применяем модель :
2.1. Какие признаки используются
2.2. Какие модели применяются
2.3. Где поднимаются модельки
2.4. Что идет на выходе модели?
2.5. Есть ли интерпретация полученного прогноза?
2.6. Какие метрики эффективности прогноза?

3. Как-то передаем прогноз
3.1. В каком виде передается прогноз?
3.2. Как часто обновляется стейт? (связано с 2)
3.3. Где сервим визуалиацию?
3.4. Дизайн визуализации
3.5. Сравнение с бейзлайном на графике

##### User Story 1 (со стороны фронта)
Пользователь заходит на сайт xxx.abc - открывается интерфейс со следующими элементами:
- список макроэкономических показателей, которые мы можем предсказывать (дропдаун бокс)
- 
- 6 месяцев вперёд
- на месяц вперёд (если дни)


Данные
- до 2008 года - другая природа данных, поэтому после 2009 года
- за последние полгода - часовые данные. В день тиковых данных проходит 2 миллиона.

##### User Story 2 (со стороны бекенда)
1) 1-2-3-4-5-6 месяцев вперёд
2) 6 чисел в виде предикта
3) На 1 месяц вперёд, но генерятся фичи, которые нужны.
Делаем инференс + аутпут модели. К этим данным применяем метод класса фичагенерации.
На это прин
==
Запускается модель, пишется предикт.
Данные, по которым строятся фичи.
Всё это будет оттуда же и выводиться.
Запросы из JS через ClickHouse, либо прослойку на FastAPI.
==
Интеграция с фронтом - с Алексеем. Сконцентрироваться на загрузке данных. Лучше на FastAPI даже если мы лишаемся каких-то 
возможнстей ClickHouse.


==
Юзать гитхаб или гитлаб - можно туда перелить код.
Пул задач.
